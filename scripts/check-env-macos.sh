#!/bin/bash
# ==============================================================================
# PHP Birthday - Environment Auditor (macOS)
# Validates that the macOS host machine meets all requirements (Homebrew, PHP).
...
echo -e "${CYAN}=========================================================${NC}"
echo -e "${CYAN} PHP Birthday - Environment Audit (macOS)                ${NC}"
echo -e "${CYAN}=========================================================${NC}"
...
if ! command -v php &> /dev/null; then
    fail "PHP is not installed or not in PATH." "Run 'brew install php' to install it via Homebrew."
    PHP_CMD="false"
else
    # 2. Check Local INI
    ENV_DIR="$DIR/../.php-env"
    ...
    if $PHP_CMD -r "exit(version_compare(PHP_VERSION, '8.4.0', '>=') ? 0 : 1);" 2>/dev/null; then
        ...
        PHP_VER=$($PHP_CMD -r "echo PHP_VERSION;" 2>/dev/null)
        fail "PHP version is too old ($PHP_VER)." "PHP 8.4.0 or higher is required. Run 'brew upgrade php'."
    fi

    if $PHP_CMD -m 2>/dev/null | grep -i -q "xdebug"; then
        pass "PHP Extension: Xdebug is loaded."
    else
        fail "Xdebug extension is not loaded." "Run 'brew tap shivammathur/extensions' followed by 'brew install shivammathur/extensions/xdebug'."
    fi
...
if command -v composer &> /dev/null; then
    pass "Composer: Found globally installed via Homebrew."
elif [ -f "$ENV_DIR/composer.phar" ]; then
    pass "Composer: Found locally in .php-env/."
else
    fail "Composer is missing." "Run 'brew install composer' to install it globally on your Mac."
fi

echo -e "${CYAN}=========================================================${NC}"
if [ "$ALL_PASSED" = true ]; then
    echo -e "${GREEN} SUCCESS: Your macOS environment is ready for testing! ${NC}"
else
